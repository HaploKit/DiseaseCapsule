...
def als_mlp(lr, optimizer, dense_layer_sizes, dropout_rate, act):
    eps = 1e-5
    x = Input(shape=(top_k, ))
    d = x
    for i, layer_size in enumerate(dense_layer_sizes):
        if layer_size == 0:
            continue
        d = Dense(layer_size,**dense_kwargs)(d)
        d = BatchNormalization(epsilon=eps)(d)
        d = Activation(act)(d)
...
lr_sched = step_decay_schedule(initial_lr=param_mlp['lr'], decay_factor=0.7, step_size=4)

# initial_learning_rate=param_mlp['lr']
...
#model.save('DNN_result/mlp.all_chr.top'+str(top_k)+'.h5')

#with open('DNN_result/mlp.all_chr.top'+str(top_k)+'.history.pkl','wb') as fw:
